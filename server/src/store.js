...
const ROUND_PRESETS = {
  ...
  [PHASES.JACKPOT]: { seconds: 60, allowedValues: [] }, // can be custom
...
function setPhase(phase) {
  ...
  if (phase === PHASES.EASY) store.game.state.clueValue = 10
  if (phase === PHASES.AVERAGE) store.game.state.clueValue = 20
...
export function setBetByProctor({ proctorId, teamId, bet }) {
  ...
  if (
    store.game.state.betsOpen &&
    bt?.key &&
    bt.eligibleTeamIds.includes(teamId)
  ) {
...
export function submitTieBreakerCorrect({
  ...
  if (
    (tb.candidateTeamIds || []).length &&
    !tb.candidateTeamIds.includes(teamId)
  ) {
